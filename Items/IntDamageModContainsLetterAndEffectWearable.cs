...
{
    public class IntDamageModContainsLetterAndEffectWearable : BaseWearableSO
    {
        // dmg mod vars
        ...
        // scrabble vars
        public char _targetLetter = 'a';

        public bool _stackBonuses = false;
        ...
        public override void TriggerPassive(object sender, object args)
        {
            ...
            if (args is DamageDealtValueChangeException context)
            {
                ...
                {
                    if (char.ToLower(c) == char.ToLower(_targetLetter))
                    {
                        finalInt += _integerToModify;
                        if (!_stackBonuses) { break; }
                    }
                } 
            }
            ...
            if (_useSimpleInt)
            {
                ...
                {
                    ex.AddModifier(new AdditionValueModifier(dmgDealt: false, finalInt, _roundNegatives));
                }
            }
            else if (_useHealing)
            {
                if (_useDealt)
                {
                    ...
                    {
                        ex2.AddModifier(new AdditionValueModifier(dmgDealt: true, finalInt, _roundNegatives));
                    }
                }
                else if (args is HealingReceivedValueChangeException ex3 && !ex3.Equals(null))
                {
                    ex3.AddModifier(new AdditionValueModifier(dmgDealt: false, finalInt, _roundNegatives));
                }
            }
            else if (_useDealt)
            {
                ...
                {
                    ex4.AddModifier(new AdditionValueModifier(dmgDealt: true, finalInt, _roundNegatives));
                }
            }
            else if (args is DamageReceivedValueChangeException ex5 && !ex5.Equals(null))
            {
                ex5.AddModifier(new AdditionValueModifier(dmgDealt: false, finalInt, _roundNegatives));
            }
        }
