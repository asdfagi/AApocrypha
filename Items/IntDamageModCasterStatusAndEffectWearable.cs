...
{
    public class IntDamageModCasterStatusAndEffectWearable : BaseWearableSO
    {
        // dmg mod vars
        ...
        public override void TriggerPassive(object sender, object args)
        {
            ...
            {
                if (context.casterUnit is CharacterCombat casterCH)
                {
                    foreach (IStatusEffect status in casterCH.StatusEffects)
                    {
                        if (status.StatusID == _statusID)
                        {
                            finalInt += _integerToModify * status.StatusContent;
                        }
                    }
                }
            }
