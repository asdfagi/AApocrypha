...
{
    public class DamagePercentIfStatusModAndSecondaryEffect_Item : BaseItem
    {
        public PercentDamageIfStatusModAndEffectWearable item;
        ...
        public string StatusID
        {
            set
            {
                item._statusID = value;
            }
        }

        public DamagePercentIfStatusModAndSecondaryEffect_Item(string itemID = "DefaultID_Item", int percentage = 1, bool useDealt = false, bool useInt = false, bool doesIncreaseDmg = false, string status = "Focused_ID")
        {
            item = ScriptableObject.CreateInstance<PercentDamageIfStatusModAndEffectWearable>();
            item._percentageToModify = percentage;
            item._useDealt = useDealt;
            item._useSimpleInt = useInt;
            item._doesIncrease = doesIncreaseDmg;
            item._statusID = status;
