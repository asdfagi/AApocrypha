...
namespace A_Apocrypha.CustomOther
{
    ...
    public class SpecificAlliesByMPassiveIDTargeting : BaseCombatTargettingSO
    {
        ...
        {
            if (m_passiveID == null || slotOffsets == null)
                return [];
            ...
            if (!isCasterCharacter)
            {
                foreach (var en in enemies.Values)
                {
                    ...
                    if (passives.Count == 0 || !en.ContainsPassiveAbility(m_passiveID))
                        continue;
            ...
            else
            {
                foreach (var ch in chars.Values)
                {
                    ...
                    if (passives.Count == 0 || !ch.ContainsPassiveAbility(m_passiveID))
                        continue;
