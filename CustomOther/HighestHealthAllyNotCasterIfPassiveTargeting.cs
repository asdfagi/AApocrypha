...
{
    public class HighestHealthAllyNotCasterIfPassiveTargeting : BaseCombatTargettingSO
    {
        ...
        public override TargetSlotInfo[] GetTargets(SlotsCombat slots, int casterSlotID, bool isCasterCharacter)
        {
            ...
            if (!isCasterCharacter)
            {
                ...
                foreach (var en in enemies.Values)
                {
                    ...
                    bool matchesPassiveWant = false;
                    if (_blacklist)
                    {
                        if (en.ContainsPassiveAbility(_passive.m_PassiveID)) { matchesPassiveWant = false; } else { matchesPassiveWant = true; }
                    }
                    else
                    {
                        if (passives.Count == 0 || !en.ContainsPassiveAbility(_passive.m_PassiveID)) { matchesPassiveWant = false; } else { matchesPassiveWant = true; }
                    }

                    if (en.CurrentHealth > checkerHealth)
                    {
                        filteredEnemies = [];
                        if (matchesPassiveWant) { filteredEnemies.Add(en); }
