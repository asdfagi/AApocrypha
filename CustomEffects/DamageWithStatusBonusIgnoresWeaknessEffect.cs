...
{
    public class DamageWithStatusBonusIgnoresWeaknessEffect : EffectSO
    {
        [DeathTypeEnumRef]
        ...
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            foreach (TargetSlotInfo targetSlotInfo in targets)
            {
                if (targetSlotInfo.HasUnit)
                {
                    ...
                    if (caster.ContainsStatusEffect("Weakness_ID"))
                    {
                        if (caster is EnemyCombat casterEN)
                        {
                            foreach (IStatusEffect status in casterEN.StatusEffects)
                            {
                                if (status.StatusID == "Weakness_ID")
                                {
                                    amount += status.StatusContent;
                                }
                            }
                        }
                        else if (caster is CharacterCombat casterCH)
                        {
