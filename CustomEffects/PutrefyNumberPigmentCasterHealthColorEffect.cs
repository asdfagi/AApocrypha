...
{
    public class PutrefyNumberPigmentCasterHealthColorEffect : EffectSO
    {
        public ManaColorSO _mana;
        public bool _useCasterHealthColor = true;
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            for (int i = 0; i < manaBarSlots.Length; i++)
            {
                ...
                bool flag = manaBarSlot != null && !manaBarSlot.IsEmpty && manaBarSlot.ManaColor != _mana && !manaBarSlot.ManaColor.SharesPigmentColor(_mana);
                bool flag2 = (AApocrypha.CrossMod.IntoTheAbyss ? manaBarSlot.ManaColor != LoadedDBsHandler.PigmentDB.GetPigment("White") : true);
                if (flag && flag2)
                {
                    list.Add(i);
                }
            ...
            while (list.Count > 0 && list2.Count < entryVariable)
            {
                ...
                ManaColorSO current = Pigments.GetPigmentWithID(stats.MainManaBar.ManaBarSlots[list[index]].ManaColor.pigmentID);
                ManaColorSO result = Pigments.SplitPigment([current, _mana]);
                
                list2.Add(list[index]);
                stats.MainManaBar.ManaBarSlots[list[index]].SetMana(result);
                list.RemoveAt(index);
                list3.Add(result);
                exitAmount++;
            }
