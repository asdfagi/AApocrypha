...
{
    public class StatusEffect_ApplyWithRandomDistribution_Effect : EffectSO
    {
        // thanks MillieAmp
        ...
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            foreach (KeyValuePair<IUnit, int> applypair in applyTo)
            {
                ...
                {
                    StatusEffect_Apply_Effect doStatus = ScriptableObject.CreateInstance<StatusEffect_Apply_Effect>();
                    doStatus._Status = status;
                    EffectInfo[] fuck =
                    [
                        Effects.GenerateEffect(doStatus, applypair.Value, Targeting.Slot_SelfAll),
                    ];
